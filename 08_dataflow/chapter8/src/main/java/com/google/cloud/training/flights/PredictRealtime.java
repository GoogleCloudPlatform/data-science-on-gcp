...
public class PredictRealtime {
  ...
  //
  // public ParseFlights(
  // PCollectionView<Map<String, Double>> delays,
  // boolean streaming) {
  // super();
  // this.delays = delays;
  ...
  // Double d = c.sideInput(delays)
  // .get(f.fromAirport + ":" + f.depHour);
  ...
  // c.outputWithTimestamp(f,
  // toInstant(fields[0], fields[13]));
  // }
  //
  // if (!streaming || fields[21].length() > 0) {
  ...
  // .getLogger(PredictRealtime.class);
  //
  ...
  // .as(MyOptions.class);
  // boolean streaming = options.getInput()
  // .contains("/topics/");
  // if (streaming) {
  // LOG.info(
  // "Creating real-time pipeline that reads from Pub/Sub I/O");
  ...
  // PCollectionView<Map<String, Double>> delays = getAverageDelays(
  // p, options.getDelayPath());
  //
  ...
  // } else {
  // // batch, from text
  // lines = p.apply("ReadLines",
  // TextIO.Read.from(options.getInput()));
  // }
  //
  // PCollection<Flight> flights = lines
  // .apply("ParseFlights", ParDo.withSideInputs(delays)
  // .of(new ParseFlights(delays, streaming))) //
  // ;
  //
  ...
  // ParDo.withSideInputs(arrDelay)
  // .of(new DoFn<Flight, String>() {
  //
  // // FIXME: distribute predictions to different machines
  // // transient TensorflowModel tfModel = new
  // // TensorflowModel(options.getModelfile(),
  ...
  // String key = "arr_" + f.toAirport + ":"
  // + f.date + ":" + (f.depHour - 1);
  // Double delay = c.sideInput(arrDelay)
  // .get(key);
  // f.averageArrivalDelay = (delay == null)
  // ? 0 : delay;
  //
  // // predict
  // boolean ontime = true; // tfModel.predict(f.getInputFeatures())
  // // > 0.5;
  //
  // // output
  ...
  //
  // if (streaming) {
  // pred.apply("WriteFlights", PubsubIO.<String> write()
  // .topic(options.getOutput()));
  // } else {
  // pred.apply("WriteFlights",
  // TextIO.Write.to(options.getOutput() + "flights")
  // .withSuffix(".csv"));
  // }
  //
  // p.run();
  ...
  //
  // @SuppressWarnings("serial")
  // private static PCollectionView<Map<String, Double>> getAverageDelays(
  // Pipeline p, String path) {
  // return p
  // .apply("Read delays-*.csv",
  // TextIO.Read.from(path + "delays-*.csv")) //
  // .apply("Parse delays-*.csv", ParDo
  // .of(new DoFn<String, KV<String, Double>>() {
  // @ProcessElement
  // public void processElement(ProcessContext c)
  // throws Exception {
  // String line = c.element();
