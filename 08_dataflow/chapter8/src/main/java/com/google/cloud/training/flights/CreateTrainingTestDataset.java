...
public class CreateTrainingTestDataset {
  ...
  // public static class ParseFlights
  // extends DoFn<String, Flight> {
  ...
  // f.date = fields[0];
  //
  // boolean isTrainDay = c.sideInput(traindays)
  // .containsKey(f.date);
  // if (!isTrainDay) {
  // LOG.debug("Ignoring " + f.date
  // + " as it is not a trainday");
  // return;
  // }
  //
  ...
  // c.output(f);
  // } catch (Exception e) {
  // LOG.warn("Malformed line {" + line + "} skipped",
  // e);
  // }
  // }
  //
  ...
  // .getLogger(CreateTrainingTestDataset.class);
  //
  ...
  // PCollectionView<Map<String, String>> traindays = getTrainingDays(
  // p, options.getTraindayCsvPath());
  //
  // PCollection<Flight> flights = p //
  // .apply("ReadLines",
  // TextIO.Read.from(options.getInput())) //
  // .apply("ParseFlights",
  // ParDo.withSideInputs(traindays)
  // .of(new ParseFlights(traindays))) //
  // ;
  //
  // PCollection<KV<String, Double>> delays = flights.apply(
  // "airport:hour",
  // ParDo.of(new DoFn<Flight, KV<String, Double>>() {
  //
  // @ProcessElement
  // public void processElement(ProcessContext c)
  // throws Exception {
  ...
  // .apply(Mean.perKey());
  //
  // delays.apply("DelayToCsv",
  // ParDo.of(new DoFn<KV<String, Double>, String>() {
  // @ProcessElement
  // public void processElement(ProcessContext c)
  // throws Exception {
  ...
  // })) //
  // .apply("WriteDelays",
  // TextIO.Write.to(options.getOutput() + "delays")
  // .withSuffix(".csv"));
  //
  // PCollectionView<Map<String, Double>> avgDelay = delays
  // .apply(View.asMap());
  // flights = flights.apply("AddDelayInfo",
  // ParDo.withSideInputs(avgDelay)
  // .of(new DoFn<Flight, Flight>() {
  //
  // @ProcessElement
  // public void processElement(ProcessContext c)
  // throws Exception {
  // Flight f = c.element().newCopy();
  // String key = f.fromAirport + ":"
  // + f.depHour;
  // Double delay = c.sideInput(avgDelay)
  // .get(key);
  // f.averageDepartureDelay = (delay == null)
  // ? 0 : delay;
  // key = "arr_" + f.toAirport + ":" + f.date
  // + ":" + (f.depHour - 1);
  // delay = c.sideInput(avgDelay).get(key);
  // f.averageArrivalDelay = (delay == null) ? 0
  // : delay;
  // c.output(f);
  // }
  //
  ...
  //
  // // write test data also, but using departure delay statistics computed on
  // // training data
  // PCollectionView<Map<String, String>> testdays = getTestDays(
  // p, options.getTraindayCsvPath());
  // PCollection<Flight> testflights = p //
  // .apply("ReadLinesT",
  // TextIO.Read.from(options.getInput())) //
  // .apply("ParseFlightsT",
  // ParDo.withSideInputs(testdays)
  // .of(new ParseFlights(testdays))) //
  // ;
  // PCollectionView<Map<String, Double>> avgArrDelay = testflights
  // .apply("airport:hour", ParDo
  // .of(new DoFn<Flight, KV<String, Double>>() {
  // @ProcessElement
  // public void processElement(ProcessContext c)
  // throws Exception {
  // Flight f = c.element();
  // // only arrival delay!
  // String key = "arr_" + f.toAirport + ":"
  // + f.date + ":" + f.arrHour;
  ...
  // ParDo.withSideInputs(avgDelay, avgArrDelay)
  // .of(new DoFn<Flight, Flight>() {
  // @ProcessElement
  // public void processElement(ProcessContext c)
  // throws Exception {
  ...
  // + f.depHour;
  // Double delay = c.sideInput(avgDelay)
  // .get(key); // from training data
  // f.averageDepartureDelay = (delay == null)
  // ? 0 : delay;
  // key = "arr_" + f.toAirport + ":" + f.date
  // + ":" + (f.depHour - 1);
  // delay = c.sideInput(avgArrDelay).get(key); // from test data
  // // (causal)
  // f.averageArrivalDelay = (delay == null) ? 0
  // : delay;
  // c.output(f);
  ...
  //
  // @SuppressWarnings("serial")
  // private static void writeCsv(PCollection<Flight> flights,
  // String prefix, MyOptions options) {
  // flights.apply("ToCsv",
  // ParDo.of(new DoFn<Flight, String>() {
  // @ProcessElement
  // public void processElement(ProcessContext c)
  // throws Exception {
  // Flight f = c.element();
  ...
  // })) //
  // .apply("WriteFlights",
  // TextIO.Write.to(options.getOutput() + prefix)
  // .withSuffix(".csv"));
  // }
  //
  // @SuppressWarnings("serial")
  // private static PCollectionView<Map<String, String>> getDaysThatMatch(
  // Pipeline p, String path, String fieldValue) {
  // return p
  // .apply("Read trainday.csv", TextIO.Read.from(path)) //
  // .apply("Parse trainday.csv", ParDo
  ...
  // }
  //
  // private static PCollectionView<Map<String, String>> getTrainingDays(
  // Pipeline p, String path) {
  // return getDaysThatMatch(p, path, "True");
  // }
  //
  // private static PCollectionView<Map<String, String>> getTestDays(
  // Pipeline p, String path) {
