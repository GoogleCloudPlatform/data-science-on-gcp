...
public class TensorflowModel {
  ...
  // s = session.Run(new StringTensorPairVector(new String[]{"save/Const:0"},
  // new Tensor[]{fn}),
  // new StringVector(), new StringVector("save/restore_all"), new
  // TensorVector());
  // if (!s.ok()) {
  // throw new RuntimeException(s.error_message().getString());
  // }
  ...
  // private static String gsDownload(String modelFile) throws IOException,
  ...
  // if (!localFile.exists()) {
  // int exitCode = Runtime.getRuntime().exec("gsutil cp " + modelFile + " " +
  // localFile).waitFor();
  // if (exitCode != 0) {
  // throw new FileNotFoundException(modelFile + " could not be downloaded. Does
  // it exist?");
  // }
  // }
  ...
  // Tensor inputs = new Tensor(tensorflow.DT_FLOAT, new
  // TensorShape(npatterns,predictors.length));
  // FloatBuffer x = inputs.createBuffer();
  // x.put(predictors);
  // Tensor keepall = new Tensor(tensorflow.DT_FLOAT, new
  // TensorShape(npatterns,1));
  ...
  //
  // // run prediction
  // Status s = session.Run(new StringTensorPairVector(new String[]
  // {"Placeholder", "Placeholder_2"}, new Tensor[] {inputs, keepall}),
